...
while dn == False:
    for event in pygame.event.get():
        ...
        if event.type==KEYDOWN:
            ...
            if event.key==K_r:x=randint(z,Z)
y=z
u=v=0
r=5
cg=w
...
f=s
gs=c
            if event.key==K_SPACE and f>0:v=v-a
f=f-5
cl=w
cr=w
ss=se                
            if event.key==K_LEFT and f>0:u=u+a
f=f-5
cl=w
ss=se
            if event.key==K_RIGHT and f>0:u=u-a
f=f-5
cr=w
ss=se
